...
class RNNLM_Model(LanguageModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(
            tf.int32, [None, self.config.num_steps], "input_placeholder")
        self.labels_placeholder = tf.placeholder(
            tf.int32, [None, self.config.num_steps], "labels_placeholder")
    ...
    def add_embedding(self):
        ...
        with tf.device('/cpu:0'):
            # YOUR CODE HERE
            all_embeddings = tf.nn.embedding_lookup(
                self.embeddings, self.input_placeholder)
            embedding_list = tf.split(all_embeddings, self.config.num_steps, 1)
            inputs = [tf.squeeze(item, squeeze_dims=(1,))
    ...
    def add_projection(self, rnn_outputs):
        ...
        self.U = tf.get_variable(
            "U", [self.config.hidden_size, len(self.vocab)])
        self.b_2 = tf.get_variable('b_2', [len(self.vocab)])
        outputs = [
            tf.matmul(tf.nn.dropout(state, self.dropout_placeholder), self.U) + self.b_2 for state in rnn_outputs]
    ...
    def add_loss_op(self, output):
        ...
        loss = sequence_loss(output,
                             ...
                             tf.constant(
                                 1.0, shape=(self.config.batch_size, self.config.num_steps))
                             )
    ...
    def __init__(self, config):
        ...
        # output = tf.reshape(tf.concat(self.outputs, 1), [-1,
        # len(self.vocab)])
    ...
    def add_model(self, inputs):
        ...
        self.H = tf.get_variable(
            "H", [self.config.hidden_size, self.config.hidden_size])
        self.I = tf.get_variable(
            "I", [self.config.embed_size, self.config.hidden_size])
        ...
        for time_step in xrange(self.config.num_steps):
            state = tf.sigmoid(
                tf.matmul(state, self.H) + tf.matmul(inputs[time_step], self.I) + self.b_1)
...
def test_RNNLM():
    ...
    with tf.variable_scope('RNNLM', reuse=tf.AUTO_REUSE) as scope:
        ...
        # This instructs gen_model to reuse the same variables as the model
        # above
